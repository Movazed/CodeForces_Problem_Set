...
int n,m,k,r[505][505],c[505][505],f[505][505][11];
void solve() {
	cin>>n>>m>>k;
	if(k&1){
	...
	for(int t=1;t<=k;t++) for(int i=1;i<=n;i++) for(int j=1;j<=m;j++){
		int&v=f[i][j][t];
		v=1e9;
		if(i>1) v=min(v,f[i-1][j][t-1]+c[i-1][j]);
		if(j>1) v=min(v,f[i][j-1][t-1]+r[i][j-1]);
		if(i<n) v=min(v,f[i+1][j][t-1]+c[i][j]);
		if(j<m) v=min(v,f[i][j+1][t-1]+r[i][j]);
	}
	for(int i=1;i<=n;i++,cout<<"\n") for(int j=1;j<=m;j++) cout<<f[i][j][k]*2<<" ";
}
