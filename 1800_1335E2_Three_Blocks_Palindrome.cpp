...
int num[205],V[205][200005]={0},S[200005]={0},a[200005];
   int i,j,k,n,ans,T;
void solve() {
    	scanf("%d",&n);
    	...
    	{
    		scanf("%d",&a[i]);
			num[a[i]]++,V[a[i]][num[a[i]]]=i;
		}
		for(i=1;i<=200;i++)
		{
			for(j=1;j<=n;j++)S[j]=S[j-1]+(a[j]==i?1:0);
			for(j=1;j<=200;j++)
			{
				ans=max(ans,num[j]);
				for(k=1;2*k<=num[j];k++)ans=max(ans,2*k+S[V[j][num[j]-k+1]-1]-S[V[j][k]]);
			}
		}
		printf("%d\n",ans);
