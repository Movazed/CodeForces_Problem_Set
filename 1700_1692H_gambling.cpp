...
#include<bits/stdc++.h>
using namespace std;
int n,_,a,ans,b[200010],l,r,sum;
map<int,int>M,cnt;
int main(){
	scanf("%d",&_);
	while(_--){
		scanf("%d",&n);M.clear(),cnt.clear(),ans=0;
		for(int i=1;i<=n;i++)
        {
			scanf("%d",&a);++cnt[a],b[i]=(cnt[a]*2)-i;
			if(b[i]<b[M[a]]||cnt[a]==1) M[a]=i;//M[a]记录a的左端点在哪；
			if(b[i]-b[M[a]]>=ans)ans=b[i]-b[M[a]],l=M[a],r=i,sum=a;
		}
		printf("%d %d %d\n",sum,l,r);
	}
	return 0;
