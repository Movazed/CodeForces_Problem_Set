...
/****************** Prime Generator **********************/ 
static const int N=200005; int prime[20000010]; 
...
int n,a,s,t,d[600005],x;
vector<int> v[600005],g; queue<int> q;
void solve() {
	cin>>n;
	for(int i=1;i<=n;i++){
		cin>>a;
	...
	}cin>>s>>t,q.push(s),d[s]=1;
	while(q.size()){
		x=q.front(),q.pop();
		for(int k:v[x]) if(!d[k]) d[k]=d[x]+1,q.push(k);
	}if(!d[t]) cout<<-1,exit(0); g.push_back(t);
	for(int i=t;i!=s;) for(int k:v[i])
		if(d[k]+1==d[i]){i=k,g.push_back(k);break;}
	reverse(g.begin(),g.end()),cout<<(g.size()+1)/2<<'\n';
	for(int k:g) if(k<=n) cout<<k<<' ';
}
