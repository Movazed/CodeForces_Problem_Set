...
void solve() {
int n;cin>>n;int ans=n,j;vi p(n+1),b(n+1),a(n+1);
for(int i=1;i<=n;++i)inn >>a[i],p[a[i]]=i;
for(int i=1;i<=n;++i)if(!b[i])for(--ans,j=i;!b[j];b[j]=i,j=a[j]);
++ans;for(int i=1;i<n;++i)if(b[p[i]]==b[p[i+1]]){ans-=2;break;}
cout<<ans<<'\n';
