...
/****************** Prime Generator **********************/ 
static const int N=5e5+5; int prime[20000010]; 
...
void solve() {
		cin>>n;
		for(int i=0;i<n;i++)
			cin>>l[i]>>r[i],a[i]=l[i],b[i]=r[i];
		sort(a,a+n);sort(b,b+n);x=0;
		for(int i=0;i<n;i++)
			x=max(x,int(n-(lower_bound(b,b+n,l[i])-b-1)-(n+1-(upper_bound(a,a+n,r[i])-a))));
		cout<<n-x<<endl;
	}
