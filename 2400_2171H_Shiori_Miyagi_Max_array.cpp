...
void task() {
	...
	vi v;
	for (int i = 2; i <= n; i++) {
		int s = (m - n + i) / i * i;for (int j = s; j > 0; j -= i) {
			int c = 0;for (ll k = i; j % k == 0; k *= i) c++;int p = upper_bound(all(v), j - i) - v.begin();
			for (int k = p; k < p + c; k++) {if (k == v.size()) v.pb(j - i);
				else v[k] = j - i;}}}cout << v.size() << "\n";
}
