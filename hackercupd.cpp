...
struct I{
    int v;
    constexpr I(int x=0):v(x){}
    constexpr operator int()const{return v;}
    constexpr I& operator=(int x){v=x;return *this;}
    constexpr I& operator+=(int x){v+=x;return *this;}
    constexpr I& operator*=(int x){v*=x;return *this;}
};
struct L{
    long long v;
    constexpr L(long long x=0):v(x){}
    constexpr operator long long()const{return v;}
    constexpr L& operator=(long long x){v=x;return *this;}
    constexpr L& operator+=(long long x){v+=x;return *this;}
    constexpr L& operator*=(long long x){v*=x;return *this;}
};
struct B{
    bool v;
    constexpr B(bool x=0):v(x){}
    constexpr operator bool()const{return v;}
    constexpr B& operator=(bool x){v=x;return *this;}
};
istream& operator>>(istream&is,I&x){int t;is>>t;x.v=t;return is;}

L sol(){
    I n;cin>>n;
    L tot=(L)(1LL*(int)n*(n+1)*(n+2)/6);
    unordered_map<int,int> mp;
    I xr=0;mp[0]++;
    I i=0;
    for(; (int)i<(int)n; i=I((int)i+1)){
        I a;cin>>a;
        xr.v^=a.v;
        mp[xr.v]++;
    }
...
int32_t main(){
    ...
    I tc;cin>>tc;
    for(I t=1;(int)t<=tc.v;t=I((int)t+1)){
        cout<<"Case #"<<(int)t<<": "<<(long long)sol().v<<'\n';
    }
    return 0;
}