...
void solve() {
    ...
    for(int i = n; i; i--){
        if(d[i + 1] == d[i + 2]) d[i] = d[i + 1] + 1;
        else{
            cout << "throw " << i << endl, cin >> d[i];
    ...
    for(int i = 1; i < n; i++) if(!a[i]){
        cout << "swap " << i << endl;
        int x; cout << "throw " << i + 1<< endl, cin >> x;
        if(x == d[i + 2] + 1) a[i] = 1; else a[i] = 2;
    }
    cout << "swap "<<n - 1 <<endl;
    int x; cout << "throw " << n - 1 << endl, cin >> x;
    if(x == 2) a[n]=1;else a[n] = 2;
