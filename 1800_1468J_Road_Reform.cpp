...
struct link{int u,v,w;}l[maxn];
bool cmp(link u, link v){return u.w<v.w;}
int T,n,m,k,mw,f[maxn];
...
void solve() {
    ...
    for (int i = 1; i <= m; i++) {
        int u = ff(l[i].u), v = ff(l[i].v);
        if (u == v) continue;

        if (l[i].w > k)
            ans += (long long)l[i].w - k;
    ...
    if (ans == 0) {
        ans = LLONG_MAX;
        for (int i = 1; i <= m; i++)
            ans = min(ans, (long long)abs(l[i].w - k));
    }
