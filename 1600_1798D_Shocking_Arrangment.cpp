...
void solve() {
    ...
    for(ll i = 1; i <= n; i++){
        ...
        max1 = max(max1, a);
        min1 = min(min1, a);
        if(a >= 0)zs.push_back(a);
        else fs.push_back(a);
    }
    bool st = 0;
    sort(all(zs),greater<ll>());
    sort(all(fs));
    ll sum = 0;
    vll ans; ans.push_back(0);
    for(int i = 1; i <= n; i++){
        if(zs.size() == 0)
        {
            ans.push_back(fs.back()); fs.pop_back();

        }
        else if(fs.size() == 0)
        {
            ans.push_back(zs.back());zs.pop_back();
        }
        else
        {
            if(sum + fs.back() >= 0)
            {
                ans.push_back(fs.back()); sum += fs.back(); fs.pop_back();
            }
            else {
                ans.push_back(zs.back()); sum += zs.back(); zs.pop_back();
            }
        }
        if(sum >= max1 - min1) st = 1;
    }
        if(st){
            cout << "NO" << nl;
            return;
