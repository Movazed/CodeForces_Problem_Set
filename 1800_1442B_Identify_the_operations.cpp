...
#define M 200002
int t,n,k,m,ans,b[M],a[M],c[M];
void solve() {
        cin>>n>>k;
        ans=1;
        a[0]=a[n+1]=1e8;
        for(int i=1;i<=n;++i) cin>>m,a[i]=0,c[m]=i;
        for(int i=1;i<=k;++i) cin>>b[i],a[c[b[i]]]=i;
        for(int i=1;i<=k;++i) ans=(ans*((a[c[b[i]]-1]<a[c[b[i]]])+(a[c[b[i]]+1]<a[c[b[i]]])))%998244353;
		cout<<ans<<'\n';
}
