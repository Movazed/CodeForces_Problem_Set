...
void solve() {
    ...
    ll res1 = response(0, n >> 2, 0);
    ll res2 = response(n >> 2, n >> 1, 0);
    ll f1 = (res1 != res2);


    ll f2 = response(0, n >> 1, f1);
    ...
    if (f2 == 1) {
        for (ll k = n >> 2; k > 0; k = k >> 1) {
            if (response(0, result + k, f1) == 0) {
                result += k;
            }
        }
    } else {
        for (ll k = n >> 2; k > 0; k = k >> 1) {
            if (response(0, result + k, f1 ^ 1) == 1) {
                result += k;
            }
        }
    }
